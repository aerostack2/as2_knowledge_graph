...
void KnowledgeGraphServer::createNode(
  ...
  RCLCPP_INFO(this->get_logger(), "graph in create node %ld",
              graph_.use_count());
...
void KnowledgeGraphServer::removeNode(
  ...
  if (this->graph_->remove_node(my_node.node_name) == true) {
    RCLCPP_INFO(this->get_logger(), " successfully remove %s",
                my_node.node_name.c_str());
  }
};
...
void KnowledgeGraphServer::removeEdge(
  ...
  if (this->graph_->remove_edge(my_edge, 1) == true) {
    RCLCPP_INFO(this->get_logger(), "%s, %s, %s ", my_edge.edge_class.c_str(),
                my_edge.source_node.c_str(), my_edge.target_node.c_str());
  }
};
...
void KnowledgeGraphServer::addPropertyNode(
  ...
  for (size_t i = 0; i < my_node.properties.size(); i++) {
    ...
    if (knowledge_graph::add_property(my_node, property.key, property.value) ==
        true) {
...
void KnowledgeGraphServer::addPropertyEdge(
  ...
  for (auto &property : my_edge.properties) {
    if (knowledge_graph::add_property(my_edge, property.key, property.value) ==
        true) {
      response->resultado = 1;
    }
  }
