...
void accessMyGraph() {
  ...
  for (size_t i = 0;
       i < server_node->getKnowledgeGraph()->get_node_names().size(); i++) {
    std::cout << node_names.at(i) << std::endl;
    // for(size_t i=0;
    // i<server_node->getKnowledgeGraph()->get_node_names().size();i++){
    //   std::cout<<"There is an edges between:"<<std::endl;
    //   edge_name =
    //   server_node->getKnowledgeGraph()->get_edges(node_names.at(i),
    //   node_names.at(1+i)); std::cout<<edge_name.at(i).edge_class<<std::endl;
    // }
  }
};
